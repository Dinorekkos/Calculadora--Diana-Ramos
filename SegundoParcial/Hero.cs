...
public class Hero: Character
{
    ...
    public void createMagicHero(int level) 
    {
        ...
        this.Speed = Math.Abs(random.Next(level) - (int)(random.Next(level)/2));
    ...
    public void createBarbarianHero(int level)
    {
        Random random = new Random();
        this.Level = level; 
        this.LifePoints = (level*2) + (int)(random.Next(level)*0.5); //MIN 22 MAX 29
        this.MagicPoints = (int)(level*0) + (int)(random.Next(level) + 1); //MIN 1 MAX 12 
        this.Stamina = random.Next(level) - (int)(random.Next(level)/10); //MIN 1 MAX 11
        this.Speed = random.Next(level) - (int)(random.Next(level)) + 1; //min 1 max 12
        this.Defense = random.Next(level) - (int)(random.Next(level)); // min 0 max 11
        this.Attack = random.Next(level) - (int)(random.Next(level)) + 1; // min 0, max 11
        this.Lucky = (int)random.Next(level)/20; //min 0 max 1
        this.Live = true;
        this.Experience = 0;
    }
